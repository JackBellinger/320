...
{
	//cout << numCols << ": " << rhs.numRows << endl;
	if(numCols != rhs.numRows)
		cout<< "left hand side's number of columns != right hand side's number of rows"<<endl;
	Matrix<T> output = Matrix(numRows, rhs.numCols);
	for(int i = 0; i < numRows; i++)
		for(int j = 0; j < rhs.numCols; j++)
			for(int k = 0; k < numCols; k++)
				output[i][j] += data[i][k] * rhs[k][j]; 
	return output;
...
{
	Matrix<T> transposed = Matrix<T>(numCols, numRows);
	//cout << "trans Rows : " << transposed.numRows;
	for(int i = 0; i < numRows; i++)
		for(int j = 0/*i*/; j < numCols; j++)//j starts at i because it swaps the top and bottom triangle
			transposed[j][i] = data[i][j];
	return transposed;

	//works for square matricies
