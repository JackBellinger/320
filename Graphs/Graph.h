...
class Graph
{
	private:
		...
		std::map<int, std::vector<int>> verticies;
	...
	public:
		...
		bool addEdge(int,int); // add an edge from the first param to the second
		...
		vector<int> printBfs(int); // prints the vertices discovered by a BFS, starting at a given vertex
		vector<int> Dfs(int);
